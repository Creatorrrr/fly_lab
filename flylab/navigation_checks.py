...
def _sample(row):
    ...
    if 'forwardAxis' in r:
        if not isinstance(r['forwardAxis'],(list,tuple)) or len(r['forwardAxis'])!=3: raise ValueError('Forward XYZ axis required')
        r['forwardAxis']=[_number(x,'forward axis') for x in r['forwardAxis']]
        ...
        if not enabled:
            ...
            if (not gap and prev is not None and self._enabled(prev)
                ...
                if self._avoiding(r): self._close_resume('INTERRUPTED_BY_AVOIDANCE',r['t'])
                    ...
                    axis=prev.get('forwardAxis',[math.cos(prev['yaw']),0.,math.sin(prev['yaw'])])
                    self.pending['forwardMM']+=dx*axis[0]+dz*axis[2]
