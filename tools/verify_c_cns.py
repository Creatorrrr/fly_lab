...
def run(graph_path,out,seconds=.2):
    ...
    for role in ('control','flexor','extensor','motor_disconnected','descending_DNp09'):
        ...
        body=FlyGymBody(42,world,config_values());n=create_backend(g,backend='exp_lif_mps');decoder=TibiaDecoder(g,spec,body)
        n.set_readout_cohort(decoder.indices);trace=[];initial=body.frame()[1]['jointAngles']
        try:
            ...
            result=dict(role=role,trace=trace,initial_angles=initial,fault=body.fault,simulated_nodes=g.n,
                        neural_tick=state['neural']['tick'],physicalExecuted=True,restore_equal=bool(restore_equal),
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--graph',required=True);p.add_argument('--out',required=True);a=p.parse_args()
    raise SystemExit(0 if run(a.graph,a.out)['status']=='PASS' else 1)
