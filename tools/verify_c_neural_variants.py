...
def run(graph_path,out,ticks=1000,backend='auto'):
    ...
    for name in ('exact-exponential-held-drive-v1','exact-exponential-reset-current-v1','exact-exponential-voltage-events-v1'):
        p=LIFParameters(integration=name);cpu=ExpLIF(g,p);gpu=create_backend(g,p,backend)
        ...
        np.savez_compressed(out/(name+'-states.npz'),**{prefix+'_'+k:s[k] for prefix,s in (('cpu',a),('gpu',b)) for k in (*errors,*discrete)})
        write_json(out/(name+'-events.json'),dict(cpu=aevents,gpu=bevents,capture=capture.tolist()))
        row=dict(integration=name,parameters=asdict(p),max_absolute_error=errors,discrete_equal=discrete,
                 ...
                 total_spikes_gpu=int(b['spike_count'].sum()),status='PASS' if passed else 'FAIL')
        rows.append(row);print(row,flush=True)
        del cpu,gpu
    report=dict(status='PASS' if all(r['status']=='PASS' for r in rows) else 'FAIL',cases=rows,
        backend=backend,graph_hash=g.hash,simulated_nodes=g.n,neural_ticks=ticks,seconds=ticks*.0001,
...
if __name__=='__main__':
    ...
    p.add_argument('--out',required=True);p.add_argument('--ticks',type=int,default=1000);p.add_argument('--backend',choices=BACKEND_CHOICES,default='auto');a=p.parse_args()
    raise SystemExit(0 if run(a.graph,a.out,a.ticks,a.backend)['status']=='PASS' else 1)
