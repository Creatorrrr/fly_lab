...
**BANC 감각 기반 신경 보행 실험**을 추가했습니다. 전체 158,706개 뉴런의 적응 발화율 모델을 실제 다리 감각과 연결해, 외부 CPG나 하행 자극 없이 10초 평지 개발 시험에서 6.64mm 이동했습니다. C 화면의 **BANC 신경망 보행 연구 → 실험 만들기 → BANC 재생**에서 사용할 수 있습니다. 기존 LIF·FlyGym 보행과는 별도 실험이며 다양한 조건의 안정성과 생물학적 타당성은 검증 중입니다. [모델·실행 결과·차단 실험·재현 방법](docs/BANC_NEURAL_WALKING_20260911.md)에 범위를 기록했습니다.

**C_STRICT 실제 점검:** 현재 Windows의 감각 프로파일 7개에서 28조건 운동 연결 대조와 기록·재생을 확인하고, 감각 차단 캠페인·Windows 한글 기록 읽기·상태 표시를 수정했습니다. 좌우 먹이 30초 실험에서는 자연 탐색 기준에 미달했습니다. [실제 결과와 원인, 수정 범위](docs/C_STRICT_AUDIT_20260911.md)에 입력 미도달·운동 출력 0·직접 자극 반응을 구분해 기록했습니다.

**몸통 고정·힘줄 구동·여러 초파리의 공유 공간**을 선택적 실험으로 추가했습니다. C 화면의 새 신체 옵션과 직접 구동 패널에서 사용할 수 있습니다. 복부 힘줄은 중복 위치 서보를 대체합니다. BANC 신경 출력도 힘줄 8입력에 연결했습니다. 발마디는 장힘줄 근육 주석을 사용하며, 복부는 목표 근육이 미확정인 MNad26 집단 출력 가설입니다. [신경 연결의 범위·한계와 실제 검증](docs/C_NEURAL_TENDONS_20260911.md)을 확인하세요. 공식 `GPUSimulation`과 기존 Warp 배치·CPU를 비교하는 도구도 추가했습니다. [사용 방법과 실제 검증 결과](docs/C_BODY_EXTENSIONS_20260911.md)에 보행 대조, BANC 고정 신체 진단, 저장·복원과 GPU 동등성 미달 결과를 기록했습니다.
...
힘줄을 사용하지 않는 BANC 다리 프로파일의 관절 표에는 축별 신경 출력 매핑과 양·음 방향 운동뉴런 수를 표시합니다. `frame.neuromuscular.actuation`은 실제 신체 순서로 매핑을 반환하며, 42축 모델은 41연결/1미연결, 전신78축은 41연결/37미연결입니다. 전체805 운동뉴런 중391개가 이 공학적 위치 변환에 사용됩니다. 해부학적 부위 주석만 있는 나머지414개를 자동으로 구동기에 배정하지 않습니다. 매핑 존재와 실제 신경 활성·행동 성공은 구분합니다.
