#!/usr/bin/env python3
"""Actual full-graph CPU/GPU comparison and fixed-input neural-dt refinement."""
import argparse
...
def run(graph_path,binding_path,out,seconds=.05,backend='auto'):
    ...
    for name,indices,amplitude in [('quiet',[],0.),('odor',sensory,18.),('descending',b.motor_indices,20.)]:
        ...
        cpu=create_backend(g);gpu=create_backend(g,backend=backend);first=None;maxima={k:0. for k in ('v','h','queue','rate')};events=[]
        for _ in range(round(seconds/.005)):
            ...
            events.append(dict(tick=cpu.tick,cpu=cpu.last_events,gpu=gpu.last_events))
    ...
    result=dict(schema='flylab.numerics.v1',graph_hash=g.hash,binding_hash=b.hash,physicalExecuted=False,
                ...
                convergence_gate='MEASURED_NOT_UNIVERSAL_APPROVAL',backend=backend,
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--graph',default='data/fafb783/bundle');p.add_argument('--bindings',default='data/fafb783/bindings.json');p.add_argument('--out',required=True);p.add_argument('--backend',choices=BACKEND_CHOICES,default='auto');p.add_argument('--seconds',type=float,default=.05);a=p.parse_args();run(a.graph,a.bindings,a.out,a.seconds,a.backend)
